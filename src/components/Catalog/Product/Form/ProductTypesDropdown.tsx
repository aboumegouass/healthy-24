...
import { product_types_data } from '../../../../utils/makeData';

function ProductTypesDropdown({ form, t }: { form: FormikProps<ProductFormData>, t: TFunction<"translation", undefined> }) {
    ...
    const shouldFilterOptions = product_types_data.every((item) => item.name !== search);
    const filteredOptions = shouldFilterOptions
        ? product_types_data.filter((item) => item.name.toLowerCase().includes(search.toLowerCase().trim()))
        : product_types_data;
    ...
    return (
        <>
                ...
                <div className='w-[calc(100%-39px)]'>
                    <Combobox
                        ...
                        onOptionSubmit={(val: any) => {
                            form.setFieldValue('product_type_id', val.id)
                            setSearch(val.name)
                    ...
                    >
                        <Combobox.Target>
                            <InputBase
                                ...
                                placeholder={`${t("Search in product types")}...`}
                                value={search}
                                onChange={(event) => {
            ...
            {form.values.product_type_id == '2' && <div className='flex items-center gap-1 mt-2'>
                <div className='w-[calc(100%-39px)]'>
                    <Combobox
                        store={categoryCombobox}
                        onOptionSubmit={(val: any) => {
                            form.setFieldValue('category_id', val.id)
                            setCateSearch(val.name)
