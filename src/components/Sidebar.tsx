...
import { NavLink, ScrollArea, useCombobox } from '@mantine/core';
            ...
            <div className={`p-2 h-full w-full`}>
                {sidebarMenu.map((elm) => (
                    <Link
                        key={elm.id}
                        to={elm.href}
                        className={`rounded-lg flex items-center gap-2 p-4 ${elm.href == location.pathname ? "font-bold bg-[#EFF3FA] opacity-100 text-[#192252]" : "opacity-75"}`}
